 ...
 *     openshadow-server             ← shell wrapper（设置 SHADOW_HOME 并启动）
 *     openshadow              ← shell wrapper（server-first CLI）
...
// 复制 node 二进制到 dist
// Windows 上改名为 openshadow-server.exe，让 main.cjs 的 bundled server 检测能命中，
// 同时在任务管理器中显示为 openshadow-server.exe 而非 node.exe（便于 NSIS 安装脚本按名杀进程）
...
// 打包模式下 SHADOW_HOME 指向 resources/server，不能依赖 renderer asar 里的 assets。
...
// RegExp external（如 /^@earendil-works\//、/^@mariozechner\//）不在此检查范围内，
...
if (isWin) {
  fs.writeFileSync(
    path.join(outDir, "openshadow-server.cmd"),
    '@echo off\r\nset "SHADOW_HOME=%~dp0"\r\nset "SHADOW_SERVER_ENTRY=%~dp0bundle\\index.js"\r\n"%~dp0openshadow-server.exe" "%~dp0bootstrap.js" %*\r\n',
  );
  fs.writeFileSync(
    path.join(outDir, "openshadow.cmd"),
    '@echo off\r\nset "SHADOW_HOME=%~dp0"\r\nset "SHADOW_SERVER_ENTRY=%~dp0bundle\\index.js"\r\n"%~dp0openshadow-server.exe" "%~dp0bundle\\cli.js" %*\r\n',
  );
} else {
  const wrapper = path.join(outDir, "openshadow-server");
  fs.writeFileSync(wrapper, [
    "#!/bin/sh",
    'DIR="$(cd "$(dirname "$0")" && pwd)"',
    'export SHADOW_HOME="$DIR"',
    'export SHADOW_SERVER_ENTRY="$DIR/bundle/index.js"',
  ...
  const cliWrapper = path.join(outDir, "openshadow");
