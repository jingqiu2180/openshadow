...
export class AppError extends Error {
  ...
  constructor(code: string, opts: { message?: string; context?: Record<string, unknown>; traceId?: string; cause?: unknown } = {}) {
    const def: Record<string, any> = ERROR_DEFS[code as keyof typeof ERROR_DEFS] || ERROR_DEFS.UNKNOWN;
  ...
  static wrap(err: unknown, fallbackCode = 'UNKNOWN'): AppError {
    ...
    if (typeof (raw as any).retryable === 'boolean') appErr.retryable = (raw as any).retryable;
    return appErr;
  }
}
