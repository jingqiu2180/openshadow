...
function registerIpcHandlers() {
    ...
    ipcMain.handle('wizard:test-connection', async (_e, providerInput) => {
        ...
        if (!spec)
            return { ok: false, error: `Unknown provider: ${providerInput.id}` };
    ...
    function sendBrowserCommand(cmd, timeoutMs = 30000) {
    ...
    ipcMain.handle('browser:wait-for', async (_e, selector, timeout) => sendBrowserCommand({ type: 'waitForSelector', selector, timeout: timeout ?? 10000 }));
