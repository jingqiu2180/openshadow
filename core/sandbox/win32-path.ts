 ...
 *
 * 将 Git Bash/MSYS/Cygwin 路径拼写转换为原生 Windows 绝对路径。
 * PathGuard 必须检查原生对象路径，而不是 shell 使用的拼写。
 */
...
function trimQuotes(value: unknown): string {
...
function toWin32Separators(value: string): string {
  return value.replace(/\//g, '\\')
}

function drivePath(drive: string, tail: string = ''): string {
...
function userHomeWin32(env: Record<string, string | undefined> = process.env): string | null {
...
/**
 * 将 Windows shell 路径拼写转换为原生绝对 Windows 路径。
 */
export function normalizeWin32ShellPath(
  rawPath: string | undefined,
  cwd: string | undefined,
  opts: { allowRelative?: boolean; env?: Record<string, string | undefined> } = {}
): string | null {
...
export function isWin32AbsoluteShellPath(rawPath: string): boolean {
  return normalizeWin32ShellPath(rawPath, undefined, { allowRelative: false }) !== null
}
