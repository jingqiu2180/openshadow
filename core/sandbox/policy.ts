...
/** agentDir 下只读的目录（通用机制） */
export const READ_ONLY_AGENT_DIRS: string[] = []
...
function uniqueTruthy(paths: unknown[]): string[] {
  const out: string[] = []
  const seen = new Set<string>()
  for (const raw of paths || []) {
    if (!raw) continue
    const value = path.resolve(String(raw))
