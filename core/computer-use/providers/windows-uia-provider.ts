...
export function createWindowsUiaProvider(_opts: any = {}) {
  const providerId = _opts.providerId ?? "windows:uia";
  const platform = _opts.platform ?? process.platform;
  const command = _opts.command ?? defaultPowerShellCommand();
  const runner = _opts.runner ?? createCommandRunner();
  const helperScript = _opts.helperScript ?? WINDOWS_UIA_HELPER_SCRIPT;
  const helperDir = _opts.helperDir ?? defaultHelperDir();
  const timeoutMs = _opts.timeoutMs ?? 30000;
  const maxResultBytes = _opts.maxResultBytes ?? DEFAULT_MAX_RESULT_BYTES;
  let helperPath = null;
