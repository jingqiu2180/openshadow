...
export function createMacosCuaProvider(_opts: any = {}) {
  const providerId = _opts.providerId ?? "macos:cua";
  const platform = _opts.platform ?? process.platform;
  const command = _opts.command ?? resolveCuaDriverCommand();
  const cursorStyle = _opts.cursorStyle ?? HANA_CUA_CURSOR_STYLE;
  const cursorImagePath = _opts.cursorImagePath ?? null;
  const cursorBloomColor = _opts.cursorBloomColor ?? HANA_CURSOR_BLOOM_COLOR;
  const cursorEnabled = _opts.cursorEnabled ?? true;
  const cursorMotion = _opts.cursorMotion ?? HANA_CURSOR_MOTION;
  const runner = _opts.runner ?? createCommandRunner();
  const timeoutMs = _opts.timeoutMs ?? 30000;
  const launchRetryAttempts = _opts.launchRetryAttempts ?? 3;
  const launchRetryDelayMs = _opts.launchRetryDelayMs ?? 350;
  const socketPath = _opts.socketPath ?? (process.env[HANA_AGENT_SOCKET_PATH_ENV] || defaultHanaComputerUseSocketPath());
  const autoStartDaemon = _opts.autoStartDaemon ?? null;
  const daemonStartupTimeoutMs = _opts.daemonStartupTimeoutMs ?? 5000;
