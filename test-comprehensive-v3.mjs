...
async function test() {
  ...
  try {
    ...
    const inputArea = await page.$('.input-area textarea, textarea, [contenteditable]');
    let test2Detail = '';
    let msgCount = 0;
    if (inputArea) {
      ...
      const storeInfo = await page.evaluate(() => {
        ...
        if (!store) return { hasStore: false, msgCount: 0, sessions: 0 };
        const currentPath = store.currentSessionPath;
        const sessionKeys = Object.keys(store.chatSessions || {});
        if (!currentPath || !store.chatSessions?.[currentPath]) {
          return { hasStore: true, msgCount: 0, currentPath, sessionKeys, allSessions: Object.keys(store.sessions || []) };
        }
        const items = store.chatSessions[currentPath]?.items || [];
        return { hasStore: true, msgCount: items.length, currentPath, sessionKeys, itemTypes: items.map(i => i.type) };
      }).catch(e => ({ hasStore: false, msgCount: 0, error: e.message }));
      msgCount = storeInfo.msgCount || 0;
      test2Detail = `store消息数:${msgCount}, hasStore:${storeInfo.hasStore}`;
      if (storeInfo.error) test2Detail += `, error:${storeInfo.error}`;
      if (storeInfo.sessionKeys) test2Detail += `, sessions:${JSON.stringify(storeInfo.sessionKeys)}`;
      if (storeInfo.allSessions) test2Detail += `, allSessions:${JSON.stringify(storeInfo.allSessions)}`;
      if (storeInfo.itemTypes) test2Detail += `, types:${JSON.stringify(storeInfo.itemTypes)}`;
      console.log('  storeInfo:', JSON.stringify(storeInfo));
    } else {
      test2Detail = '未找到输入框';
    }
    results.push({ test: '聊天消息渲染', pass: msgCount > 0, detail: test2Detail });
    ...
    const inputArea2 = await page.$('.input-area textarea, textarea, [contenteditable]');
    let inputPass = false;
    if (inputArea2) {
